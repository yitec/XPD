...
/*****************************************************************************************************************
Accion:Ejecuta todas las operaciones sobre reportes
...
********************************************************/
	...
	function obtiene_pagos($parametros,$hoy){
		$v_datos=explode(",",$parametros);
/*		$fechaini = strtotime($v_datos[0]);
		$fechaini = date('Y-m-d',$fechaini);
		$fechafin = strtotime($v_datos[1]);
		$fechafin = date('Y-m-d',$fechafin);
*/
		$ano=substr($v_datos[0], 6, 4);
		$mes=substr($v_datos[0], 0, 2);
		$dia=substr($v_datos[0], 3, 2);
		$fechaini=$ano."-".$mes."-".$dia." 00:00:00";
		$ano=substr($v_datos[1], 6, 4);
		$mes=substr($v_datos[1], 0, 2);
		$dia=substr($v_datos[1], 3, 2);
		$fechafin=$ano."-".$mes."-".$dia." 00:00:00";	
		$result=mysql_query("select * from tbl_cobros where fecha_pago>='".$fechaini."' and fecha_pago<='".$fechafin."' and estado=1 ");
//		$result=mysql_query("select * from tbl_cobros");		
	...
	********************************************************/
	...
	/*******************************************************
	accion="convierte una fecha"
	parametros="fecha a converti"
	invocacion= "todos los reportes con fechas"
	/*******************************************************/
	...
	return $fecha_ini=$ano."-".$mes."-".$dia." 00:00";
